...
class BaseModel():
    ...
    def load_network(self, network, network_label, strict=True):
        if self.opt['path']['resume_path'] is None:
            self.logger.info('NO------------------loading pretrained model')
            return 
        self.logger.info('Beign loading pretrained model [{:s}] ...'.format(network_label))
        
        
        if self.opt['path']['resume_path'] is not None:
            self.logger.info('--------------------load resume_path:')
            model_path = "{}".format(self.opt['path']['resume_path'])
        else:
            self.logger.info('--------------------load resume_path:[{:s}]'.format(self.opt['path']['resume_path']))
            self.logger.info('--------------------load resume_state--------------------:')
